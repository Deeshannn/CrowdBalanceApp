...
const getMissingReportsByStatus = async (req, res, next) => {
    ...
    if (!['Active', 'Found', 'Closed'].includes(status)) {
        return res.status(400).json({ message: "Invalid status. Use 'Active', 'Found', or 'Closed'" });
    }
    ...
    if (!reports || reports.length === 0) {
        return res.status(404).json({ message: `No ${status.toLowerCase()} missing reports found!` });
    }
...
const getMissingReportsByUserId = async (req, res, next) => {
    ...
    if (!reports || reports.length === 0) {
        return res.status(404).json({ message: "No missing reports found for this user!" });
    }
...
const createMissingReport = async (req, res, next) => {
    const { name, age, gender, image, lastseenlocation, description, UserId } = req.body;

    // Validation
    if (!name || !age || !gender || !image || !lastseenlocation || !description || !UserId) {
        return res.status(400).json({ 
            message: "All fields are required: name, age, gender, image, lastseenlocation, description, UserId" 
        });
    }
    ...
    try {
        ...
        const missingReport = new MissingReport({
            ...
            description: description.map(desc => desc.trim()),
            UserId
        });
...
const getMissingReportById = async (req, res, next) => {
    ...
    try {
        report = await MissingReport.findById(id)
            .populate('UserId', 'name email userType');
    } catch (err) {
        console.log(err);
        return res.status(500).json({ message: "Server error" });
...
const updateMissingReport = async (req, res, next) => {
    const { id } = req.params;
    const { name, age, gender, image, lastseenlocation, description, status } = req.body;
    ...
    if (!report) {
        return res.status(404).json({ message: "Unable to update missing report!" });
    }
...
const deleteMissingReport = async (req, res, next) => {
    ...
    try {
        report = await MissingReport.findByIdAndDelete(id)
            .populate('UserId', 'name email userType');
    } catch (err) {
        console.log(err);
        return res.status(500).json({ message: "Server error" });
    ...
    if (!report) {
        return res.status(404).json({ message: "Unable to delete missing report!" });
    }
...
const searchMissingReports = async (req, res, next) => {
    ...
    if (!reports || reports.length === 0) {
        return res.status(404).json({ message: "No matching missing reports found!" });
    }
